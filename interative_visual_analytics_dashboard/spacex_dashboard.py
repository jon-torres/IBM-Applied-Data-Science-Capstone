...
def create_scatter(site, payload):
    ...
    if site == "ALL":
        filter_df = spacex_df[
            (spacex_df["Payload Mass (kg)"] >= int(payload[0]))
            & (spacex_df["Payload Mass (kg)"] <= int(payload[1]))
        ]
        ...
        filter_df = filter_df[
            (filter_df["Payload Mass (kg)"] >= int(payload[0]))
            & (filter_df["Payload Mass (kg)"] <= int(payload[1]))
        ]
        print(filter_df[["class", "Payload Mass (kg)"]])
        fig = px.scatter(
            filter_df,
